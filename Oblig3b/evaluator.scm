...
(define (special-form? exp)
  (cond ((quoted? exp) #t)
        ...
        ((cond? exp) #t)
        #| ##########################################
        Sjekk for å påse at and/or er i special form
        ########################################## |#
        ((and? exp) #t)
        ((or? exp) #t)
        #| ##########################################
...
#| ###########################################################
Redefinerer evalueringen av if til å kunne bruke elsif-syntaks
########################################################### |#
...
(define (eval-if exp env)
  (define (if-checker logic-list)
    (if (true? (mc-eval (equal? (car logic-list) 'else) env))
        (mc-eval (cadr logic-list) env)
        (if (true? (mc-eval (cadr logic-list) env))
            (mc-eval (cadddr logic-list) env)
            (if-checker (cddddr logic-list)))))
  (if-checker exp))
    

#| ###########################################################
Evalueringsregler for and/or
########################################################### |#
(define (eval-and exp env)
  (define (and-checker logic-list)
    (if (true? (mc-eval (null? logic-list) env))
        #t
        (if (true? (mc-eval (car logic-list) env))
            (and-checker (cdr logic-list))
            #f)))
  (and-checker (and-or-tests exp)))
  
(define (eval-or exp env)
  (define (or-checker logic-list)
    (if (true? (mc-eval (null? logic-list) env))
        #f
        (if (false? (mc-eval (car logic-list) env))
            (or-checker (cdr logic-list))
            #t)))
  (or-checker (and-or-tests exp)))
...
#| #############################################################################
   Sjekk om et uttrykk er av typen or/and
############################################################################# |#
(define (or? exp) (tagged-list? exp 'or))
(define (and? exp) (tagged-list? exp 'and))
#| #############################################################################
   Abstraksjons av selektører for and/or. Tar ut en liste av de logiske testene
############################################################################# |#
